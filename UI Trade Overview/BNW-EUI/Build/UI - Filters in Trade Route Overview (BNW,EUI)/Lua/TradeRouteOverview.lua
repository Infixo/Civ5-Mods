...
function RegisterSortOptions()
	...
	-- VP Controls.bTroeBlocked:RegisterCheckHandler( OnTroeFilterChecked );
	-- TROE
end
...
function OnTroeFilterChecked(bIsChecked)
	--[[print("Filters are:",
		...
		-- VP Controls.bTroeBlocked:IsChecked());--]]
	DisplayData();
end
-- TROE
-------------------------------------------------------------------------------
...
function TabSelect(tab)
	...
	-- VP Controls.bTroeBlocked:SetHide(g_CurrentTab ~= "AvailableTR");
	-- TROE
...
function SetData(data)
	...
	for i,v in ipairs(data) do
		...
		-- TROE
		v.TradeConnectionType = 0; -- can't be nil anyway
		if v.FromID == v.ToID then
			if v.ToFood > 0 then           v.TradeConnectionType = TradeConnectionTypes.TRADE_CONNECTION_FOOD;
			elseif v.ToProduction > 0 then v.TradeConnectionType = TradeConnectionTypes.TRADE_CONNECTION_PRODUCTION;
			else print("ERROR: Internal route has both Food and Production equal to 0.");
			end
		else
			v.TradeConnectionType = TradeConnectionTypes.TRADE_CONNECTION_INTERNATIONAL;
		end
		-- TROE
	end
	g_Data = data;
...
function DisplayData()
	...
	-- VP local bTroeBlocked	= Controls.bTroeBlocked:IsChecked();
    ...
    for i,v in ipairs(g_Data) do
		...
		-- VP local sToRouteName = v.ToCityName.."#"..v.ToCivilizationType;
		if g_CurrentTab == "AvailableTR" then -- run filters only in this page
			...
			if v.TradeConnectionType == TradeConnectionTypes.TRADE_CONNECTION_PRODUCTION and bTroeProduction then bIncludeInternal = true; end  -- VP v.TradeConnectionType == TradeConnectionTypes.TRADE_CONNECTION_WONDER_RESOURCE
			if v.TradeConnectionType == TradeConnectionTypes.TRADE_CONNECTION_INTERNATIONAL then
				...
				-- VP if not bTroeBlocked and tAlreadyActiveRoutes[ sToRouteName ] then bIncludeBlocked = false; end
			end
		end		
		if (g_CurrentTab ~= "AvailableTR") or  -- filters active only in Available TRs page
		   (g_CurrentTab == "AvailableTR" and bIncludeDomain and (bIncludeInternal or bIncludeForeign)) then  -- VP and bIncludeBlocked
		-- TROE
        local instance = {};
		...
		-- VP TROE add red exclamation mark for already targeted INT cities
